...
export class LeaderboardService {
  ...
  async getLeaderboard(getLeaderboardDto: GetLeaderboardDto): Promise<LeaderboardEntry[]> {
    const { type, period, schoolId, courseId, category, limit = 10, offset = 0, includeInactive = false } = getLeaderboardDto;

    // Try to get cached leaderboard first
    const cachedLeaderboard = await this.getCachedLeaderboard(type as LeaderboardType, period as LeaderboardPeriod, {
      schoolId,
      courseId,
      category,
    });
    ...
    const entries = await this.generateLeaderboard(type as LeaderboardType, period as LeaderboardPeriod, {
      schoolId,
      courseId,
      category,
      includeInactive,
    ...
    // Cache the result
    await this.cacheLeaderboard(type as LeaderboardType, period as LeaderboardPeriod, entries, {
      schoolId,
      courseId,
      category,
    });
  ...
  private async generateLeaderboard(
    ...
    if (!includeInactive) {
      query.lastActivityDate = { $gte: new Date(Date.now() - 30 * 24 * 60 * 60 * 1000) };
    }
    ...
    if (userPoints.length === 0) {
      this.logger.log('No users found with points, getting first 20 registered users');
    ...
    for (let i = 0; i < userPoints.length; i++) {
      ...
      const entry: LeaderboardEntry = {
        ...
        points: period === LeaderboardPeriod.ALL_TIME ? userPoint.totalPoints : this.getPeriodPoints(userPoint, period),
        level: userPoint.level,
        rank: i + 1,
        previousRank: await this.getPreviousRank(user._id.toString(), type, period, filters),
  ...
  private getPeriodRange(period: LeaderboardPeriod): { startDate: Date; endDate: Date } {
    const now = new Date();
  ...
  private getPeriodPoints(userPoint: UserPointsDocument, period: LeaderboardPeriod): number {
    const now = new Date();
  ...
  private async getPreviousRank(
    ...
    const cachedLeaderboard = await this.getCachedLeaderboard(type, previousPeriod, filters);
  ...
  private async getUserBadgeCount(userId: string, schoolId?: string): Promise<number> {
    try {
      // Import the UserAchievement model to count badges
      const { UserAchievement } = await import('../schemas/user-achievement.schema');
      const userAchievementModel = this.userPointsModel.db.model('UserAchievement');
      ...
      const badgeCount = await userAchievementModel.countDocuments(query).exec();
      return badgeCount;
    } catch (error) {
