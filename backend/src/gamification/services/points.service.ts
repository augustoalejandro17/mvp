...
import { AwardPointsDto, DeductPointsDto, UpdateStreakDto } from '../dto/points.dto';
...
export class PointsService {
  ...
  async initializeUserPoints(userId: string, schoolId: string): Promise<UserPointsDocument> {
    ...
    const level1 = await this.levelModel.findOne({ level: 1, isActive: true }).exec();
  ...
  async awardPoints(awardPointsDto: AwardPointsDto): Promise<{
    ...
    const { userId, schoolId, points, actionType, description, courseId, classId, badgeId, metadata, sendNotification } = awardPointsDto;
    ...
    // Send notifications
    if (sendNotification) {
      await this.sendPointsNotification(userId, points, actionType, description);
    }
  ...
  async getUserPoints(userId: string, schoolId: string): Promise<UserPoints | null> {
    let userPoints = await this.userPointsModel
      .findOne({ user: userId, school: schoolId })
      .exec();
  ...
  async findUserPointsRecord(userId: string): Promise<UserPointsDocument | null> {
    return this.userPointsModel
      .findOne({ user: userId })
      .exec();
  }

  async getTopUsers(schoolId: string, limit: number = 10): Promise<UserPoints[]> {
    return this.userPointsModel
  ...
  async getUserRank(userId: string, schoolId: string): Promise<{
    schoolRank: number;
    globalRank: number;
    ...
    const schoolRank = await this.userPointsModel
      .countDocuments({
        school: schoolId,
    ...
    const globalRank = await this.userPointsModel
      .countDocuments({
        totalPoints: { $gt: userPoints.totalPoints },
        isLeaderboardVisible: true,
      })
  ...
  private async sendLevelUpNotification(userId: string, level: LevelInfo): Promise<void> {
    try {
      ...
      this.logger.error(`Failed to send level up notification: ${error.message}`);
    }
  }
  ...
  async seedDefaultLevels(): Promise<void> {
    ...
    for (const levelData of defaultLevels) {
      const existingLevel = await this.levelModel.findOne({ level: levelData.level }).exec();
      if (!existingLevel) {
