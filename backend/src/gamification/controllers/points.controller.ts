...
export class PointsController {
  ...
  @UseGuards(RolesGuard)
  @Roles(UserRole.TEACHER, UserRole.ADMIN, UserRole.SUPER_ADMIN, UserRole.SCHOOL_OWNER)
  async getSchoolPointsStats(
    ...
    return {
      ...
      averagePoints: topUsers.length > 0 
        ? topUsers.reduce((sum, user) => sum + user.totalPoints, 0) / topUsers.length 
        : 0,
    };
  }
  ...
  async testGetUser(
    ...
    try {
      ...
      const existingPoints = await this.pointsService.getUserPoints(userId, schoolId);
      return {
        status: 'success',
        userId,
        schoolId,
  ...
  async getPublicUserPoints(
    ...
    try {
      ...
      if (!schoolId) {
        const userPointsRecord = await this.pointsService.findUserPointsRecord(userId);
        if (userPointsRecord) {
