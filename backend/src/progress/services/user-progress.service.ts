...
import { UserClassProgress, UserClassProgressDocument } from '../schemas/user-class-progress.schema';
import { UserCourseProgress, UserCourseProgressDocument } from '../schemas/user-course-progress.schema';
import { Class, ClassDocument } from '../../classes/schemas/class.schema';
import { Course, CourseDocument } from '../../courses/schemas/course.schema';
import { Playlist, PlaylistDocument } from '../../classes/schemas/playlist.schema';
...
export class UserProgressService {
   ...
   */
  async updateClassProgress(data: ProgressUpdateData): Promise<UserClassProgressDocument> {
    try {
      const {
        userId,
        classId,
      ...
      if (isNowCompleted && !wasCompleted) {
        ...
        this.logger.log(`Class ${classId} marked as completed for user ${userId}`);
      }
  ...
  private isClassCompleted(classProgress: UserClassProgressDocument): boolean {
    ...
    return (
      ...
      (classProgress.attendanceMarked && classProgress.attendanceStatus !== 'absent')
    );
  }
   ...
   */
  async updateCourseProgress(userId: string, courseId: string, schoolId: string): Promise<UserCourseProgressDocument> {
    try {
      ...
      const completedClasses = classProgresses.filter(cp => cp.completed).length;
      const completionPercentage = totalClasses > 0 ? Math.round((completedClasses / totalClasses) * 100) : 0;
      const attendedClasses = classProgresses.filter(cp => 
      ...
      const averageVideoWatchPercentage = classProgresses.length > 0
        ? Math.round(classProgresses.reduce((sum, cp) => sum + cp.videoWatchPercentage, 0) / classProgresses.length)
        : 0;
      ...
      courseProgress.longestStreak = Math.max(courseProgress.longestStreak, longestStreak);
  ...
  async getTotalVideoMinutesInCourse(courseId: string): Promise<number> {
    try {
      ...
      this.logger.error(`Error getting total video minutes for course ${courseId}: ${error.message}`);
      return 0;
    }
  }
   ...
   */
  private calculateStreaks(classProgresses: UserClassProgressDocument[]): { streak: number; longestStreak: number } {
   ...
   */
  async getUserCourseProgress(userId: string, courseId: string): Promise<CourseProgressSummary | null> {
    try {
   ...
   */
  async getUserCoursesProgress(userId: string, schoolId?: string): Promise<CourseProgressSummary[]> {
    try {
      ...
      this.logger.error(`Error getting user courses progress: ${error.message}`);
      return [];
    }
  }
   ...
   */
  async initializeCourseProgress(userId: string, courseId: string, schoolId: string): Promise<UserCourseProgressDocument> {
    try {
   ...
   */
  async getUserClassProgressInCourse(userId: string, courseId: string): Promise<UserClassProgressDocument[]> {
    try {
