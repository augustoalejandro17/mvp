...
import { CreatePlaylistDto, UpdatePlaylistDto, AddClassToPlaylistDto, RemoveClassFromPlaylistDto } from './dto/create-playlist.dto';
...
export class PlaylistsService {
    ...
    for (const classItem of classes) {
      try {
        if (classItem.videoUrl) {
          ...
          this.logger.warn(`Class ${classItem._id} has no videoUrl, using original object`);
          classesWithSignedUrls.push(classItem.toObject ? classItem.toObject() : { ...classItem });
        }
      } catch (error) {
        this.logger.error(`Error generating signed URL for class ${classItem._id}: ${error.message}`, error.stack);
  ...
  async create(createPlaylistDto: CreatePlaylistDto, userId: string, userRole: string): Promise<Playlist> {
    try {
      // Verify course exists
      const course = await this.courseModel.findById(createPlaylistDto.course).populate('teacher teachers');
      if (!course) {
      ...
      if (!this.canModifyPlaylist(course, userId, userRole)) {
        throw new BadRequestException('No tienes permisos para crear listas de reproducción en este curso');
      }
      ...
      this.logger.error(`Error creating playlist: ${error.message}`, error.stack);
      throw error;
    }
  }

  async findByCourse(courseId: string, userId?: string, userRole?: string): Promise<Playlist[]> {
    try {
      let playlistQuery: any = { course: courseId, isActive: true };
      ...
      for (const playlist of playlists) {
        ...
        if (playlistObj.classes && playlistObj.classes.length > 0) {
          playlistObj.classes = await this.generateSignedUrlsForClasses(playlistObj.classes);
        }
      ...
      this.logger.error(`Error finding playlists for course: ${error.message}`, error.stack);
      throw error;
    }
  }
  ...
  async findOne(id: string): Promise<Playlist> {
    try {
      ...
      this.logger.error(`Error finding playlist: ${error.message}`, error.stack);
      throw error;
    }
  }

  async update(id: string, updatePlaylistDto: UpdatePlaylistDto, userId: string, userRole: string): Promise<Playlist> {
    try {
      ...
      if (!this.canModifyPlaylist(playlist.course, userId, userRole)) {
        throw new BadRequestException('No tienes permisos para modificar esta lista de reproducción');
      }

      const updatedPlaylist = await this.playlistModel
        .findByIdAndUpdate(id, { ...updatePlaylistDto, updatedAt: new Date() }, { new: true })
        .populate('classes')
        .populate('createdBy', 'name email')
        .exec();
      ...
      this.logger.error(`Error updating playlist: ${error.message}`, error.stack);
      throw error;
    }
  }
  ...
  async remove(id: string, userId: string, userRole: string): Promise<void> {
    try {
      ...
      if (playlist.isDefault) {
        throw new BadRequestException('No se puede eliminar la lista de reproducción por defecto');
      }

      // Check permissions
      if (!this.canModifyPlaylist(playlist.course, userId, userRole)) {
        throw new BadRequestException('No tienes permisos para eliminar esta lista de reproducción');
      }

      await this.playlistModel.findByIdAndUpdate(id, { isActive: false });
    } catch (error) {
      this.logger.error(`Error removing playlist: ${error.message}`, error.stack);
      throw error;
    }
  }
  ...
  async addClassToPlaylist(playlistId: string, addClassDto: AddClassToPlaylistDto, userId: string, userRole: string): Promise<Playlist> {
    try {
      ...
      // Check permissions
      if (!this.canModifyPlaylist(playlist.course, userId, userRole)) {
        this.logger.error(`Permission denied for user ${userId} with role ${userRole} to modify playlist ${playlistId}`);
        throw new BadRequestException('No tienes permisos para modificar esta lista de reproducción');
      }
      ...
      if (classCourseId !== playlistCourseId) {
        this.logger.error(`Course mismatch: class course ${classCourseId} vs playlist course ${playlistCourseId}`);
        throw new BadRequestException('La clase debe pertenecer al mismo curso que la lista de reproducción');
      }

      // Check if class is already in the playlist
      if (playlist.classes.includes(addClassDto.classId as any)) {
        this.logger.error(`Class ${addClassDto.classId} already in playlist ${playlistId}`);
        throw new BadRequestException('La clase ya está en esta lista de reproducción');
      }
  ...
  async removeClassFromPlaylist(playlistId: string, removeClassDto: RemoveClassFromPlaylistDto, userId: string, userRole: string): Promise<Playlist> {
    try {
      const playlist = await this.playlistModel.findById(playlistId).populate('course');
      if (!playlist) {
        throw new NotFoundException('Lista de reproducción no encontrada');
      ...
      playlist.classes = playlist.classes.filter(classId => classId.toString() !== removeClassDto.classId);
      playlist.updatedAt = new Date();
      await playlist.save();

      return await this.findOne(playlistId);
    } catch (error) {
      this.logger.error(`Error removing class from playlist: ${error.message}`, error.stack);
      throw error;
    }
  }

  async createDefaultPlaylist(courseId: string, userId: string): Promise<Playlist> {
    try {
      ...
      const defaultPlaylist = new this.playlistModel({
        ...
        description: 'Lista de reproducción por defecto con todas las clases del curso',
        course: courseId,
        createdBy: userId,
        isDefault: true,
        order: 0,
      ...
      this.logger.error(`Error creating default playlist: ${error.message}`, error.stack);
      throw error;
    }
  }

  async getUnorganizedClasses(courseId: string): Promise<Class[]> {
    try {
      // Get all playlists for the course
      const playlists = await this.playlistModel.find({ course: courseId, isActive: true }).exec();
      ...
      const allClasses = await this.classModel.find({ course: courseId, isActive: true }).exec();
      const unorganizedClasses = allClasses.filter(cls => !organizedClassIds.has(cls._id.toString()));

      // Generate signed URLs for unorganized classes
      const unorganizedClassesWithSignedUrls = await this.generateSignedUrlsForClasses(unorganizedClasses);

      return unorganizedClassesWithSignedUrls;
    } catch (error) {
      this.logger.error(`Error getting unorganized classes: ${error.message}`, error.stack);
      throw error;
    }
  }

  private canModifyPlaylist(course: any, userId: string, userRole: string): boolean {
    ...
    if (userRole === 'teacher') {
      ...
      if (course.teachers && Array.isArray(course.teachers)) {
        return course.teachers.some(teacher => teacher._id.toString() === userId);
      }
    }
  ...
  async reorderClassesInPlaylist(playlistId: string, classIds: string[], userId: string, userRole: string): Promise<Playlist> {
    try {
      const playlist = await this.playlistModel.findById(playlistId).populate('course');
      if (!playlist) {
        throw new NotFoundException('Lista de reproducción no encontrada');
      ...
      if (classes.length !== classIds.length) {
        throw new BadRequestException('Algunas clases no existen o no pertenecen a este curso');
      }
      ...
      this.logger.error(`Error reordering classes in playlist: ${error.message}`, error.stack);
      throw error;
    }
  }
} 