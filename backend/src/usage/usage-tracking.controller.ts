...
export class UsageTrackingController {
  ...
  async getSchoolUsageSummary(
    ...
    try {
      ...
      return await this.usageTrackingService.getUsageSummary(schoolId, monthNum, yearNum);
    } catch (error) {
      this.logger.error(`Error getting usage summary for school ${schoolId}: ${error.message}`, error.stack);
      throw error;
    }
  }
  ...
  async getSchoolsWithOverages(
    ...
    try {
      ...
      return await this.usageTrackingService.getSchoolsWithOverages(monthNum, yearNum);
    } catch (error) {
      this.logger.error(`Error getting schools with overages: ${error.message}`, error.stack);
      throw error;
    }
  }
  ...
  async startStreamingSession(
    ...
    try {
      ...
      const sessionId = await this.streamingIntegrationService.startVideoStreaming(
        userId,
        body.assetId,
        body.schoolId,
        body.relatedCourse,
  ...
  async endAllUserSessions(@Req() req: any): Promise<{ success: boolean; message: string }> {
    try {
      const userId = req.user.sub || req.user._id;
  ...
  async getActiveStreamingSessions(
    ...
    try {
      ...
      this.logger.error(`Error getting active streaming sessions for school ${schoolId}: ${error.message}`, error.stack);
      throw error;
    }
  }
  ...
  async finalizeMonthlyUsage(
    ...
    try {
      await this.usageTrackingService.finalizeMonthlyUsage(body.month, body.year);
      ...
      this.logger.error(`Error finalizing monthly usage: ${error.message}`, error.stack);
      throw error;
    }
  }
  ...
  async cleanupStaleSessions(
    ...
    try {
      await this.streamingIntegrationService.cleanupStaleSessions(body.maxAgeMinutes);
  ...
  async getActiveSessionsStats(): Promise<{
    ...
    try {
      const totalActiveSessions = this.streamingIntegrationService.getActiveSessionsCount();
