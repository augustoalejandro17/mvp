      ...
      try {
        ...
        if (payload.sessionId) {
          ...
          if (!user.currentSessionId) {
            this.logger.warn(
              `Sesión inválida para usuario ${user.email}: sesión fue cerrada o invalidada`,
            );
            throw new UnauthorizedException(
              'Sesión inválida: la sesión ha sido cerrada',
            );
          }
          ...
          if (payload.sessionId !== user.currentSessionId) {
            this.logger.warn(
              `Sesión inválida para usuario ${user.email}: token de sesión ${payload.sessionId} no coincide con sesión activa ${user.currentSessionId}`,
            );
            throw new UnauthorizedException(
              'Sesión inválida: este usuario ha iniciado sesión en otro dispositivo',
            );
          }
