...
export class S3Service {
  ...
  async uploadVideo(file: Express.Multer.File): Promise<string> {
    try {
      ...
      try {
        ...
        this.logger.error(
          `Error generando URL firmada de S3: ${signError.message}`,
          signError.stack,
        );
  ...
  async getSignedUrl(
    ...
    if (!key) {
      this.logger.error(
        'getSignedUrl: Se intentó generar una URL para una key vacía',
      );
    ...
    try {
      ...
      try {
        ...
        if (
          ...
          this.logger.warn(
            `Acceso denegado al intentar eliminar el video ${key}. El archivo permanecerá en S3.`,
          );
        ...
        this.logger.error(
          `Error al eliminar video de S3 (${key}): ${s3Error.message}`,
          s3Error.stack,
        );
      ...
      this.logger.error(
        `Error al procesar la eliminación del video: ${error.message}`,
        error.stack,
      );
      // No lanzamos el error para permitir que el proceso de eliminación de la clase continúe
    }
      ...
      if (this.cloudFrontService) {
        try {
          ...
          try {
            ...
            url = await this.s3.getSignedUrlPromise(
              'getObject',
              signedUrlParams,
            );
          } catch (signError) {
  ...
  async moveProcessedVideo(
    ...
    try {
      ...
      this.logger.log(
        `Video moved from temp to final bucket: ${tempKey} -> ${finalKey}`,
      );
  ...
  async uploadProcessedVideo(
    ...
    try {
      ...
      this.logger.log(
        `Processed video uploaded: ${key}, size: ${buffer.length} bytes`,
      );
