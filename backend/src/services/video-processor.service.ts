...
export class VideoProcessorService implements OnModuleInit {
  ...
  async onModuleInit() {
    ...
    try {
      ...
      if (systemPath && fs.existsSync(systemPath)) {
        ...
        try {
          ...
          this.logger.log(
            'ffmpeg instalado en el sistema funcionando correctamente',
          );
          return;
        } catch (verifyError) {
          this.logger.warn(
            `Advertencia al verificar ffmpeg: ${verifyError.message}`,
          );
    ...
    try {
      ...
      this.logger.warn(
        `No se pudo usar el paquete @ffmpeg-installer/ffmpeg: ${error.message}`,
      );
      ...
      for (const potentialPath of commonPaths) {
        try {
          if (fs.existsSync(potentialPath)) {
            ...
            this.logger.log(
              `Encontrado ffmpeg en ubicación común: ${potentialPath}`,
            );
            break;
          }
        } catch (error) {
    ...
    if (this.ffmpegPath) {
      ...
      this.logger.error(
        '⚠️ No se pudo encontrar ffmpeg en ninguna ubicación. El procesamiento de video no funcionará.',
      );
    }
  }
  ...
  async processVideo(
    inputBuffer: Buffer,
  ): Promise<{ processedFilePath: string; cleanup: () => void }> {
    if (!this.ffmpegPath) {
      this.logger.error(
        'ffmpeg no está disponible. No se puede procesar el video.',
      );
      throw new Error(
        'Cannot find ffmpeg. Please install ffmpeg to process videos.',
      );
    }
    ...
    try {
      ...
      this.logger.log(
        `Directorio temporal ${tempDir} tiene permisos de escritura`,
      );
    } catch (err) {
      this.logger.error(
        `Error de acceso al directorio temporal ${tempDir}: ${err.message}`,
      );
      ...
      try {
        ...
        this.logger.error(
          `No se pudo crear el directorio temporal: ${mkdirErr.message}`,
        );
        throw new Error(
          `No hay directorio temporal disponible: ${mkdirErr.message}`,
        );
      }
    }
    ...
    const tempInputPath = path.join(
      tempDir,
      `input-${timestamp}-${randomId}.mp4`,
    );
    const tempOutputPath = path.join(
      tempDir,
      `output-${timestamp}-${randomId}.mp4`,
    );
    ...
    try {
      ...
      this.logger.log(
        `Archivo de entrada escrito: ${tempInputPath} (${inputBuffer.length} bytes)`,
      );
    } catch (writeErr) {
      this.logger.error(
        `Error al escribir archivo de entrada: ${writeErr.message}`,
      );
      throw new Error(
        `Error al escribir archivo de entrada: ${writeErr.message}`,
      );
    ...
    this.logger.log(
      `Procesando video... Input: ${tempInputPath}, Output: ${tempOutputPath}`,
    );
    ...
    return new Promise((resolve, reject) => {
      ...
      if (!fs.existsSync(tempInputPath)) {
        const error = new Error(
          `El archivo de entrada ${tempInputPath} no existe`,
        );
      ...
      if (useDirectCommand) {
        this.logger.log(
          'Usando comando ffmpeg directo en lugar de fluent-ffmpeg',
        );
        try {
          ...
          child_process.exec(ffmpegCmd, (error, stdout, stderr) => {
            ...
            this.logger.log(
              'Procesamiento de video completado con éxito (comando directo)',
            );
            resolve({
              ...
              cleanup: () => {
                fs.unlink(tempInputPath, (err) => {
                  if (err)
                    this.logger.error(
                      `Error limpiando archivo temporal: ${err.message}`,
                    );
                });
          ...
          this.logger.error(
            `Error con comando directo ffmpeg: ${cmdError.message}`,
          );
          if (fs.existsSync(tempInputPath)) {
            fs.unlinkSync(tempInputPath);
          }
        ...
        ffmpeg(tempInputPath)
          ...
          .on('progress', (progress) => {
            if (progress.percent) {
              this.logger.verbose(
                `Progreso del procesamiento: ${progress.percent}%`,
              );
            }
          })
          .on('end', () => {
            ...
            try {
              ...
              this.logger.log(
                `Archivo de salida generado correctamente: ${tempOutputPath} (${stats.size} bytes)`,
              );
              ...
              this.logger.error(
                `Error al verificar archivo de salida: ${statErr.message}`,
              );
              fs.unlink(tempInputPath, () => {});
              reject(statErr);
              return;
