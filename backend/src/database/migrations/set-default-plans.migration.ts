...
export class SetDefaultPlansMigration {
  ...
  async up(): Promise<void> {
    console.log(
      'Starting migration: Assigning Basic plan to existing academies...',
    );

    // Find the Basic plan
    const basicPlan = await this.planModel
      .findOne({
        type: PlanType.BASIC,
        isActive: true,
      })
    ...
    if (!basicPlan) {
      throw new Error(
        'Basic plan not found. Please ensure plans are initialized first.',
      );
    }

    // Find all schools without a planId
    const schoolsWithoutPlan = await this.schoolModel
      .find({
    ...
    console.log(
      `Found ${schoolsWithoutPlan.length} academies without assigned plans`,
    );
    ...
    for (const school of schoolsWithoutPlan) {
      ...
      console.log(
        `Updated academy ${school.name} with Basic plan (${currentSeats} current seats)`,
      );
    }
