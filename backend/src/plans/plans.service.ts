...
export class PlansService {
  ...
  async initDefaultPlans() {
    ...
    if (plansCount === 0) {
      ...
      const defaultPlans = Object.values(PLAN_CONFIGS).map(config => ({
        ...
        overageStreamingCentsPerHour: config.overUsageUnitPrices.streamingCentsPerHour,
        ...
        extraStreamingMinutesPrice: config.overUsageUnitPrices.streamingCentsPerHour / 60 / 100, // Convert hour to minute pricing
        extraCoursePerUserPrice: 5.00 // Default value
      }));
      
      await this.planModel.insertMany(defaultPlans);
      console.log(`Initialized ${defaultPlans.length} default plans with authoritative pricing`);
    }
  }
   ...
   */
  async validateUserCreation(schoolId: string): Promise<{ allowed: boolean; message: string }> {
    const school = await this.schoolModel.findById(schoolId).populate('planId').exec();
    if (!school) {
    ...
    if (school.currentSeats >= totalLimit) {
      // Record overage
      await this.recordOverage(schoolId, school.planId.toString(), OverageType.STUDENT, 1);
      return {
        allowed: false,
   ...
   */
  async validateFileUpload(schoolId: string, fileSizeGB: number): Promise<{ allowed: boolean; message: string }> {
    const school = await this.schoolModel.findById(schoolId).populate('planId').exec();
    if (!school) {
      throw new BadRequestException('School not found');
    }
    ...
    if (newUsage > totalLimit) {
      ...
      // Record overage
      await this.recordOverage(schoolId, school.planId.toString(), OverageType.STORAGE, overage);
      return {
        allowed: false,
   ...
   */
  async validateStreamingSession(schoolId: string, estimatedHours: number): Promise<{ allowed: boolean; message: string }> {
    const school = await this.schoolModel.findById(schoolId).populate('planId').exec();
    if (!school) {
      throw new BadRequestException('School not found');
    }
    ...
    if (newUsage > totalLimit) {
      const overage = newUsage - totalLimit;
      // Record overage
      await this.recordOverage(schoolId, school.planId.toString(), OverageType.STREAMING, overage);
      return {
        allowed: false,
   ...
   */
  async validateCourseEnrollment(schoolId: string, studentId: string): Promise<{ allowed: boolean; message: string }> {
    const school = await this.schoolModel.findById(schoolId).populate('planId').exec();
    if (!school) {
      throw new BadRequestException('School not found');
    }
   ...
   */
  async updateSchoolUsage(schoolId: string, usage: {
    seats?: number;
    storageGB?: number;
    ...
    if (usage.storageGB !== undefined) updateData.usedStorageGB = usage.storageGB;
    if (usage.streamingHours !== undefined) updateData.usedStreamingHours = usage.streamingHours;
   ...
   */
  async assignPlanToAcademy(academyId: string, planType: PlanType): Promise<any> {
    const plan = await this.planModel.findOne({ type: planType, isActive: true }).exec();
    if (!plan) {
      throw new BadRequestException(`Plan type ${planType} not found or inactive`);
    }
    ...
    await this.schoolModel.findByIdAndUpdate(academyId, { 
      planId: plan._id 
    }).exec();
   ...
   */
  async assignPlanToAcademyById(academyId: string, planId: string): Promise<any> {
    ...
    return {
      ...
      planDetails: {
        name: plan.name,
        type: plan.type,
        monthlyPrice: formatPriceFromCents(plan.monthlyPriceCents || (plan.monthlyPrice * 100) || (plan.price * 100) || 0),
        limits: {
          studentSeats: plan.studentSeats || plan.maxUsers || 0,
          teachers: plan.teachers || 2,
          storageGB: plan.storageGB || plan.maxStorageGb || 0,
          streamingHours: plan.streamingHoursPerMonth || (plan.maxStreamingMinutesPerMonth / 60) || 0
        }
      }
    };
  }
   ...
   */
  async grantExtraResources(academyId: string, resources: {
    extraSeats?: number;
    ...
    const updateData: any = {};
    if (resources.extraSeats !== undefined) updateData.extraSeats = resources.extraSeats;
    if (resources.extraStorageGB !== undefined) updateData.extraStorageGB = resources.extraStorageGB;
    if (resources.extraStreamingHours !== undefined) updateData.extraStreamingHours = resources.extraStreamingHours;
  ...
  async getAcademyPlanDetails(academyId: string): Promise<any> {
    const academy = await this.schoolModel.findById(academyId).populate('planId').exec();
    if (!academy) {
      throw new BadRequestException('Academy not found');
    }
    ...
    return {
      ...
      limits: {
        studentSeats: (plan.studentSeats || plan.maxUsers || 0) + (academy.extraSeats || 0),
        teachers: plan.teachers || 2,
        storageGB: (plan.storageGB || plan.maxStorageGb || 0) + (academy.extraStorageGB || 0),
        streamingHours: (plan.streamingHoursPerMonth || (plan.maxStreamingMinutesPerMonth / 60) || 0) + (academy.extraStreamingHours || 0)
      },
      usage: {
        currentSeats: calculatedCurrentSeats,
  ...
  async getAcademyOverages(academyId: string): Promise<any> {
    const overages = await this.overageModel.find({ schoolId: academyId })
      .sort({ recordedAt: -1 })
      .exec();

    const totalOverageCosts = overages.reduce((sum, overage) => sum + overage.totalPriceCents, 0);
  ...
  async validateAllLimits(academyId: string): Promise<any> {
    ...
    const streamingValidation = await this.validateStreamingSession(academyId, 0); // Check current usage
    ...
    return {
      ...
      overallStatus: userValidation.allowed && storageValidation.allowed && streamingValidation.allowed ? 'WITHIN_LIMITS' : 'EXCEEDING_LIMITS'
    };
  }
