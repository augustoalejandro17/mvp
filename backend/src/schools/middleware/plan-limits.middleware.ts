...
export class PlanLimitsMiddleware implements NestMiddleware {
    ...
    if (this.shouldCheckLimits(method, path)) {
      try {
        ...
        if (this.isAddingUser(method, path)) {
          const result = await this.subscriptionsService.canAddUserToSchool(schoolId);
          ...
          const courseId = req.body.courseId || this.extractCourseIdFromPath(req);
          
          if (userId && courseId) {
            const result = await this.subscriptionsService.canEnrollUserToCourse(userId, courseId);
  ...
  private shouldCheckLimits(method: string, path: string): boolean {
    ...
    return (
      (method === 'POST' && /\/api\/schools\/.*\/(students|teachers|administratives)/.test(path)) ||
