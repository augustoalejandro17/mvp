...
export class SchoolsController {
  ...
  async findAll(@Req() req) {
    ...
    const userId =
      req.user.sub || (req.user._id ? req.user._id.toString() : null);
    const userRole = req.user.role;

    this.logger.log(
      `Buscando escuelas para usuario: ${userId}, rol: ${userRole}`,
    );
  ...
  async findOne(@Param('id') id: string) {
    ...
    try {
      ...
      this.logger.error(
        `Error al obtener escuela con ID ${id}: ${error.message}`,
      );
      throw error;
    }
  }
  ...
  async update(
    ...
    try {
      const result = await this.schoolsService.update(
        id,
        updateSchoolDto,
      ...
      this.logger.error(
        `Error al actualizar escuela: ${error.message}`,
        error.stack,
      );
      throw error;
    }
  ...
  async addTeacher(
    ...
    try {
      const result = await this.schoolsService.addTeacher(
        id,
        teacherId,
        req.user._id,
      );
  ...
  async addStudent(
    ...
    try {
      const result = await this.schoolsService.addStudent(
        id,
        studentId,
        req.user._id,
      );
  ...
  async remove(@Param('id') id: string, @Req() req) {
    ...
    try {
      ...
      this.logger.error(
        `Error al eliminar escuela: ${error.message}`,
        error.stack,
      );
      throw error;
    }
  ...
  async getTeachersBySchool(@Param('id') id: string, @Req() req) {
    ...
    try {
      ...
      this.logger.error(
        `Error al obtener profesores de la escuela ${id}: ${error.message}`,
        error.stack,
      );
      throw error;
    }
