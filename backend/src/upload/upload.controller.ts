import { Controller, Post, UseGuards, UploadedFile, UseInterceptors, BadRequestException, Logger, Req } from '@nestjs/common';
...
export class UploadController {
  ...
  async uploadImage(@UploadedFile() file: Express.Multer.File, @Req() req) {
    ...
    try {
      ...
      this.logger.error(`Error al subir la imagen: ${error.message}`, error.stack);
      throw error;
    }
  }
} 