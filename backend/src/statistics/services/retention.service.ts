...
export class RetentionService {
  ...
  async getRetentionRatesByCourse(): Promise<RetentionRateDto[]> {
    ...
    for (const course of courses) {
      ...
      const enrollments = await this.enrollmentModel.find({ course: course._id }).exec();
      ...
      // Tasa de retención
      const retentionRate = initialEnrollment > 0 
      ...
      const completionRate = initialEnrollment > 0 
