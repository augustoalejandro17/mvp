...
export class StatisticsService {
  ...
  async getOverviewStats(
    ...
    const { from, to, academyIds } = await this.validateAndParseQuery(
      query,
      user,
    );

    const snapshots = await this.dailySnapshotModel
      .find({
    ...
    return {
      ...
      attendanceRate: this.calculateAttendanceRate(
        aggregated.presentCount,
        aggregated.absentCount,
      ),
  ...
  async getProfessorMetrics(
    ...
    const professorIds = Array.from(professorMap.keys()).filter(
    ...
    const professorNames = new Map(
      professors.map((p) => [p._id.toString(), p.name]),
    );
  ...
  async getCourseMetrics(
    ...
    const courseIds = Array.from(courseMap.keys()).filter(
      (id) => id !== 'null' && id !== 'undefined',
    );
  ...
  async getCategoryMetrics(
    ...
    const categoryIds = Array.from(categoryMap.keys()).filter(
      (id) => id !== 'null' && id !== 'undefined' && id !== 'uncategorized',
    );
    ...
    return Array.from(categoryMap.entries()).map(([id, data]) => ({
      ...
      name:
        id === 'uncategorized'
          ? 'Uncategorized'
          : categoryNames.get(id) || 'Unknown Category',
      value: this.calculateMetricValue(query.metric, data),
      context: {
        present: data.present,
        absent: data.absent,
  ...
  private async validateAndParseQuery(query: StatisticsQueryDto, user: any) {
    ...
    if (userRoleStr.includes('super') || userRoleStr.includes('admin')) {
      ...
      if (query.academyId) {
        ...
        const allSchools = await this.dailySnapshotModel
          .distinct('academyId')
          .exec();
    ...
    if (
      normalizedUserRole === UserRole.SUPER_ADMIN ||
      ...
      if (query.academyId) {
        ...
        throw new BadRequestException(
          'School owners must specify academyId parameter',
        );
      }
    } else if (
      ...
      if (query.academyId) {
        ...
        throw new BadRequestException(
          'Administrative users must specify academyId parameter',
        );
      }
    } else {
  ...
  private calculateMetricValue(metric: MetricType, data: any): number {
    switch (metric) {
      ...
      case MetricType.RETENTION:
        return data.retentionCount > 0
          ? data.retentionSum / data.retentionCount
          : 0;
      default:
        return 0;
    }
  ...
  private createEmptyOverviewStats(
    from: Date,
    to: Date,
  ): OverviewStatsResponseDto {
    return {
