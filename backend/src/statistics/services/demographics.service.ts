...
export class DemographicsService {
  ...
  async getAgeDistribution(): Promise<AgeDistributionDto> {
    ...
    const age18to25Percentage = 0.4; // 40% están entre 18 y 25
    const age26to35Percentage = 0.3; // 30% están entre 26 y 35
    const over36Percentage = 0.1; // 10% son mayores de 36
  ...
  async getAgeDistributionByCourse() {
    ...
    const coursesWithStudents = await this.userModel
      .aggregate([
    ...
    return coursesWithStudents.map((course) => {
      ...
      const under18 = Math.round(
        course.totalStudents * (0.15 + Math.random() * 0.05),
      );
      const age18to25 = Math.round(
        course.totalStudents * (0.4 + Math.random() * 0.05),
      );
      const age26to35 = Math.round(
        course.totalStudents * (0.3 + Math.random() * 0.05),
      );
      const over36 = Math.round(
        course.totalStudents * (0.1 + Math.random() * 0.05),
      );
      const unknown =
        course.totalStudents - (under18 + age18to25 + age26to35 + over36);
