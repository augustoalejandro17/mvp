...
export class ReportsService {
  ...
  async getMonthlyAttendanceReport(
    ...
    this.logger.log(
      `Generating monthly attendance report for ${userRole} user ${userId}`,
    );

    // 1. Determine which schools the user can access
    const accessibleSchoolIds = await this.getAccessibleSchools(
      userId,
      userRole,
      schoolId,
    );
    ...
    if (accessibleSchoolIds.length === 0) {
      throw new BadRequestException(
        'No accessible schools found for this user',
      );
    }
    ...
    const courseFilter: any = { school: targetSchoolId };
    if (courseId) {
    ...
    const courseAttendanceData = await Promise.all(
      courses.map((course) =>
        this.getCourseAttendanceData(course, startDate, endDate),
      ),
    );
    ...
    if (courseId && courses.length === 1) {
      ...
      const detailedCourseData = await Promise.all(
        courses.map((course) =>
          this.getDetailedCourseAttendanceData(course, startDate, endDate),
        ),
      );
    ...
    this.logger.log(
      `Generated report for ${courses.length} courses with ${summary.totalClasses} total classes`,
    );
    return report;
  }

  private async getAccessibleSchools(
    userId: string,
    userRole: string,
    schoolId?: string,
  ): Promise<string[]> {
    ...
    switch (userRole.toLowerCase()) {
      ...
      case 'administrative':
        const administratedSchools = await this.schoolModel.find({
    ...
    if (schoolId && !accessibleSchoolIds.includes(schoolId)) {
      throw new BadRequestException(
        'User does not have access to the requested school',
      );
    }
  ...
  private async getCourseAttendanceData(
    ...
    const uniqueDates = [
      ...new Set(attendanceRecords.map((record) => record.date.toDateString())),
    ];
    const totalClasses = uniqueDates.length;

    // Calculate unique students
    const uniqueStudents = [
      ...new Set(attendanceRecords.map((record) => record.student.toString())),
    ];
  ...
  private calculateSummary(courseData: CourseAttendanceData[]) {
    ...
    const totalClasses = courseData.reduce(
    ...
    const totalPresent = courseData.reduce(
      (sum, course) => sum + course.presentCount,
      0,
    );
    const totalRecords = courseData.reduce(
      (sum, course) => sum + course.presentCount + course.absentCount,
      0,
    );
  ...
  async exportMonthlyAttendanceReport(
    ...
    if (params.format === 'csv') {
      ...
      const detailedReport =
        await this.getDetailedMonthlyAttendanceReport(params);
  ...
  async getDetailedMonthlyAttendanceReport(
    params: MonthlyReportParams,
  ): Promise<DetailedMonthlyAttendanceReport> {
    ...
    const detailedCourseData = await Promise.all(
      basicReport.courseDetails.map(async (courseData) => {
        ...
        const startDate = new Date(
          basicReport.period.year,
          basicReport.period.month - 1,
          1,
        );
        const endDate = new Date(
          basicReport.period.year,
          basicReport.period.month,
          0,
          23,
          59,
          59,
  ...
  private async getDetailedCourseAttendanceData(
    ...
    const basicData = await this.getCourseAttendanceData(
      course,
      startDate,
      endDate,
    );
      ...
      ...new Set(
        attendanceRecords.map((record) => {
          ...
          const localDate = new Date(
            record.date.getTime() - timezoneOffset * 60000,
          );
    ...
    const studentDetails: StudentAttendanceDetail[] = Array.from(
      studentAttendanceMap.values(),
    ).map((student) => {
      const totalRecords = student.totalPresent + student.totalAbsent;
      const attendancePercentage =
        totalRecords > 0
          ? Math.round((student.totalPresent / totalRecords) * 100)
          : 0;
  ...
  private createSummarySheet(
    ...
    report: DetailedMonthlyAttendanceReport,
  ): void {
    const summaryData = [
      [`Reporte de Asistencia - ${report.school.name}`],
      [`Período: ${report.period.monthName} ${report.period.year}`],
  ...
  private createCourseDetailSheet(
    workbook: XLSX.WorkBook,
    courseData: DetailedCourseAttendanceData,
    period: any,
  ): void {
  ...
  async getMonthlyPaymentReport(
    ...
    this.logger.log(
      `Generating monthly payment report for ${userRole} user ${userId}`,
    );
    ...
    this.logger.log(
      `Payment report course filter: ${JSON.stringify(courseFilter)}`,
    );
    const courses = await this.courseModel.find(courseFilter);
    ...
    if (courseId && courses.length === 1) {
      ...
      const detailedCourseData = await Promise.all(
        courses.map((course) =>
          this.getDetailedCoursePaymentData(course, month, year),
        ),
      );
      report.detailedCourseData = detailedCourseData;
    }
    ...
    this.logger.log(
      `Generated payment report for ${courses.length} courses with total revenue: ${summary.totalRevenue}`,
    );
    return report;
  }

  private async getCoursePaymentData(
    course: any,
    month: number,
    year: number,
  ): Promise<CoursePaymentData> {
    ...
    enrollments.forEach((enrollment) => {
      ...
      if (monthPayments.length > 0) {
        ...
        totalRevenue += monthPayments.reduce(
  ...
  private calculatePaymentSummary(courseData: CoursePaymentData[]) {
    ...
    const totalStudentsWithPayments = courseData.reduce(
      (sum, course) => sum + course.studentsWithPayments,
      0,
    );
  ...
  private createEmptyPaymentReport(
    school: any,
    month: number,
    year: number,
  ): MonthlyPaymentReport {
  ...
  async getDetailedMonthlyPaymentReport(
    params: MonthlyReportParams,
  ): Promise<DetailedMonthlyPaymentReport> {
    ...
    const detailedCourseData = await Promise.all(
      basicReport.courseDetails.map(async (courseData) => {
        ...
        return this.getDetailedCoursePaymentData(
          course,
          basicReport.period.month,
          basicReport.period.year,
        );
  ...
  private async getDetailedCoursePaymentData(
    ...
    const studentDetails: StudentPaymentDetail[] = enrollments.map(
      (enrollment) => {
        const studentData = enrollment.student as any;

        // Filter payments for the target month
        const monthPayments =
          enrollment.paymentHistory?.filter(
            (payment) => payment.month === targetMonth,
          ) || [];
  ...
  private generatePaymentCSV(
    report: DetailedMonthlyPaymentReport,
  ): ExportResult {
  ...
  private createPaymentSummarySheet(
    workbook: XLSX.WorkBook,
    report: DetailedMonthlyPaymentReport,
  ): void {
    const { school, period, summary, courseDetails } = report;
  ...
  private createCoursePaymentDetailSheet(
    workbook: XLSX.WorkBook,
    courseData: DetailedCoursePaymentData,
    period: any,
  ): void {
    const sheetName = courseData.courseName.substring(0, 31); // Excel sheet name limit
