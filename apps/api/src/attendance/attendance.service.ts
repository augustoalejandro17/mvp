...
export class AttendanceService {
  ...
  async create(
    ...
    if (!Types.ObjectId.isValid(studentId)) {
      throw new BadRequestException(
        `studentId inválido: ${studentId}. Debe ser un ObjectId.`,
      );
    }
    ...
    if (!isEnrolled && student.role !== UserRole.UNREGISTERED) {
      ...
      throw new BadRequestException(
        `El estudiante ${student.name} (ID: ${studentId}) no está matriculado en este curso.`,
      );
      ...
      const courseContainsStudent = course.students.some(
        (sId) => sId.toString() === studentId,
      );
      if (!courseContainsStudent) {
        throw new BadRequestException(
          `El estudiante no registrado (ID: ${studentId}) no está asociado a este curso.`,
        );
      }
    }
    ...
    const adjustedDate = new Date(
      searchDate.getTime() - timezoneOffset * 60000,
    );
    ...
    if (present) {
      try {
        ...
        this.logger.error(
          `Error awarding gamification points for attendance: ${error.message}`,
        );
  ...
  async createBulk(
    ...
    for (const attendanceData of attendances) {
      ...
      if (!Types.ObjectId.isValid(currentStudentId)) {
        this.logger.warn(
          `studentId inválido en bulk: ${currentStudentId}. Saltando este registro.`,
        );
        continue;
      }
      ...
      if (!student) {
        this.logger.warn(
          `Estudiante con ID ${currentStudentId} no encontrado en bulk. Saltando.`,
        );
        continue;
      }
      ...
      if (!isEnrolled && student.role !== UserRole.UNREGISTERED) {
        this.logger.warn(
          `Estudiante ${student.name} (ID: ${currentStudentId}) no matriculado. Saltando.`,
        );
        continue;
      } else if (!isEnrolled && student.role === UserRole.UNREGISTERED) {
        const courseContainsStudent = course.students.some(
          (sId) => sId.toString() === currentStudentId,
        );
        if (!courseContainsStudent) {
          this.logger.warn(
            `Estudiante no registrado (ID: ${currentStudentId}) no asociado a este curso. Saltando.`,
          );
          continue;
        }
      }
      ...
      if (existingAttendance) {
        ...
        if (currentPresent) {
          try {
            ...
            this.logger.error(
              `Error awarding gamification points for bulk attendance: ${error.message}`,
            );
            // Don't throw error to avoid breaking attendance recording
          }
        }
  ...
  async getStatsByCourse(courseId: string): Promise<any> {
    ...
    const totalClasses = await this.attendanceModel.distinct('date', {
      course: courseId,
    });
  ...
  async createForNonRegisteredUser(
    courseId: string,
    studentName: string,
    date: Date,
    present: boolean,
    ...
    try {
      ...
      if (!unregisteredUser) {
        try {
          ...
          if (
            ...
            !course.students.some(
              (s) => s?.toString() === unregisteredUser._id.toString(),
            )
          ) {
          ...
          try {
            await this.coursesService.enrollStudent(
              courseId,
              unregisteredUser._id.toString(),
              teacherId,
            );
          } catch (error) {
            // Si el enrollment ya existe o hay otro error no crítico, continuar
            this.logger.warn(
              `Could not create enrollment for new unregistered user: ${error.message}`,
            );
          }
        } catch (error) {
          this.logger.error(
        ...
        if (
          !unregisteredUser.enrolledCourses.some(
            (c) => c?.toString() === courseId,
          )
        ) {
          unregisteredUser.enrolledCourses.push(
            new Types.ObjectId(courseId) as any,
          );
        }

        // Verificar si la escuela ya está en schools
        if (
          !unregisteredUser.schools ||
          !unregisteredUser.schools.some((s) => s?.toString() === schoolId)
        ) {
        ...
        if (
          !unregisteredUser.schoolRoles ||
          !unregisteredUser.schoolRoles.some(
        ...
        try {
          ...
          // Si el enrollment ya existe o hay otro error no crítico, continuar
          this.logger.warn(
            `Could not create enrollment for unregistered user: ${error.message}`,
          );
        }
      }
      ...
      this.logger.error(
        `Error en createForNonRegisteredUser: ${error.message}`,
        error.stack,
      );
      throw error;
    }
  ...
  async linkAttendancesToRegisteredUser(
    ...
    if (
      unregisteredUser.enrolledCourses &&
      unregisteredUser.enrolledCourses.length > 0
    ) {
      for (const courseId of unregisteredUser.enrolledCourses) {
        if (
          !registeredUser.enrolledCourses.some(
