...
export class CoursesService {
  ...
  async create(
    ...
    if (sede) {
      if (!school.sedes || school.sedes.length === 0) {
        throw new BadRequestException(
          `School ${school.name} does not have any sedes defined. Cannot assign course to a sede.`,
        );
      }
      if (!school.sedes.includes(sede)) {
        throw new BadRequestException(
          `Sede '${sede}' is not a valid sede for school ${school.name}. Valid sedes are: ${school.sedes.join(', ')}`,
        );
      }
    }
    ...
    if (!mainTeacher) {
      throw new BadRequestException(
        `El profesor principal con ID ${teacher} no existe`,
      );
    }
    ...
    if (teachers && teachers.length > 0) {
      ...
      if (teachers.length > 4) {
        // 4 + 1 principal = 5 total
        throw new BadRequestException(
          'Un curso no puede tener más de 5 profesores',
        );
      }
      ...
      for (const teacherId of teachers) {
        if (!uniqueTeachers.has(teacherId)) {
          ...
          if (!teacher) {
            throw new BadRequestException(
              `El profesor con ID ${teacherId} no existe`,
            );
          }
    ...
    for (const tId of teachersArray) {
      try {
        ...
        this.logger.error(
          `Error al añadir curso al profesor ${tId}: ${error.message}`,
          error.stack,
        );
      }
    }
    ...
    if (scheduleTimes && scheduleTimes.length > 0) {
      try {
        ...
        this.logger.error(
          `Error creating schedule for course ${result._id}: ${error.message}`,
          error.stack,
        );
  ...
  async findAll(userId?: string, role?: UserRole | string, schoolId?: string) {
    try {
      ...
      this.logger.error(
        `Error al buscar cursos: ${error.message}`,
        error.stack,
      );
      throw error;
    }
  ...
  async findOne(id: string) {
    try {
      ...
      const courseWithVisibility = {
        ...
        classes: Array.isArray(course.classes)
          ? course.classes.map((classItem) => {
              ...
              const isPublic =
                typeof classItem === 'object' && 'isPublic' in classItem
                  ? classItem.isPublic
                  : false;
              const order =
                typeof classItem === 'object' && 'order' in classItem
                  ? classItem.order
                  : 0;
      ...
      this.logger.error(
        `Error al buscar curso ${id}: ${error.message}`,
        error.stack,
      );
      throw error;
    }
  ...
  async update(
    ...
    if (sede !== undefined) {
      // If sede is part of the update
      const targetSchoolId = schoolId || course.school.toString();
      const targetSchool = schoolId
        ? await this.schoolModel.findById(schoolId)
        : school;

      if (!targetSchool) {
        throw new NotFoundException(
          `Target school with ID ${targetSchoolId} not found for sede validation.`,
        );
      }

      if (sede === '' || sede === null) {
        // Allowing to remove/nullify sede
        course.sede = null; // Assign null directly to the course model property
      } else {
        if (!targetSchool.sedes || targetSchool.sedes.length === 0) {
          throw new BadRequestException(
            `School ${targetSchool.name} does not have any sedes defined. Cannot assign course to a sede.`,
          );
        }
        if (!targetSchool.sedes.includes(sede)) {
          throw new BadRequestException(
            `Sede '${sede}' is not a valid sede for school ${targetSchool.name}. Valid sedes are: ${targetSchool.sedes.join(', ')}`,
          );
        }
      }
    }
    ...
    if (schoolId && schoolId !== course.school.toString()) {
      ...
      if (sede) {
        if (!newSchool.sedes || newSchool.sedes.length === 0) {
          throw new BadRequestException(
            `New school ${newSchool.name} does not have any sedes defined for sede '${sede}'.`,
          );
        }
        if (!newSchool.sedes.includes(sede)) {
          throw new BadRequestException(
            `Sede '${sede}' is not valid for new school ${newSchool.name}. Valid sedes are: ${newSchool.sedes.join(', ')}`,
          );
        }
      } else {
    ...
    if (teachers)
      course.teachers = teachers.map((tId) => new Types.ObjectId(tId) as any);
      ...
      try {
        if (scheduleTimes.length === 0) {
          ...
          const existingSchedule =
            await this.courseScheduleService.getSchedule(id);
          if (existingSchedule) {
            // Update existing schedule
            await this.courseScheduleService.updateSchedule(id, {
              scheduleTimes,
              enableNotifications:
                enableNotifications ?? existingSchedule.enableNotifications,
              notificationMinutes:
                notificationMinutes || existingSchedule.notificationMinutes,
            });
          } else {
            // Create new schedule
        ...
        this.logger.error(
          `Error updating schedule for course ${id}: ${error.message}`,
          error.stack,
        );
  ...
  async remove(id: string, userId: string, userRoleString?: string) {
    try {
      ...
      const actualRole = (
        userRoleString ? String(userRoleString) : String(userRequesting.role)
      ).toLowerCase();
      ...
      // 1. Super Admin?
      if (
        actualRole === UserRole.SUPER_ADMIN.toLowerCase() ||
      ...
      if (
        ...
        if (
          school.admin &&
          school.admin.toString() === userId &&
          (actualRole === UserRole.SCHOOL_OWNER.toLowerCase() ||
            actualRole === UserRole.ADMIN.toLowerCase())
        ) {
          hasPermission = true;
        }
      ...
      if (!hasPermission) {
        ...
        throw new UnauthorizedException(
          'No tiene los permisos necesarios para eliminar este curso.',
        );
      }
      ...
      try {
        ...
        this.logger.error(
          `Error al eliminar referencia del curso en la escuela: ${error.message}`,
          error.stack,
        );
      ...
      this.logger.error(
        `Error al eliminar curso ${id}: ${error.message}`,
        error.stack,
      );
      throw error;
    }
  ...
  async enrollStudent(
    ...
    if (existingEnrollment) {
      ...
      if (!existingEnrollment.isActive) {
        ...
        if (
          !school.students ||
          !school.students.some((id) => id.toString() === studentId)
        ) {
        ...
        if (
          !student.schools ||
          !student.schools.some((id) => id.toString() === schoolId)
        ) {
    ...
    try {
      ...
      this.logger.error(
        `Error actualizando referencias: ${error.message}`,
        error.stack,
      );
  ...
  async unenrollStudent(
    ...
    const otherEnrollmentsInSameSchool = await this.enrollmentModel
      .find({
        student: new Types.ObjectId(studentId),
        isActive: true,
  ...
  async getAllEnrollments(userId: string): Promise<Enrollment[]> {
    ...
    if (
      ...
      compareRole(user.role, UserRole.SCHOOL_OWNER)
    ) {
      ...
      const courseIds = courses.map(
        (course) => new Types.ObjectId(course._id.toString()),
      );
  ...
  async getEnrollmentById(
    ...
    if (
      compareRole(userRole, UserRole.TEACHER) &&
      course.teacher.toString() === userId
    ) {
      return enrollment;
    }
    ...
    if (
      (compareRole(userRole, UserRole.ADMIN) ||
        compareRole(userRole, UserRole.SCHOOL_OWNER)) &&
      (user.administratedSchools.some(
        (id) =>
          id.toString() ===
          (course.school && typeof course.school === 'object'
            ? course.school.toString()
            : String(course.school)),
      ) ||
        user.ownedSchools.some(
          (id) =>
            id.toString() ===
            (course.school && typeof course.school === 'object'
              ? course.school.toString()
  ...
  async updateEnrollment(
    ...
    if (
      updateEnrollmentDto.paymentStatus === true &&
  ...
  async getTeachingCourses(userId: string, userRole: string) {
    try {
      ...
      if (
        compareRole(userRole, UserRole.SUPER_ADMIN) ||
        compareRole(userRole, UserRole.ADMIN)
      ) {
      ...
      this.logger.error(
        `Error getting teaching courses: ${error.message}`,
        error.stack,
      );
      throw error;
    }
    ...
    try {
      ...
      const courseIds = enrollments.map((enrollment) =>
        enrollment.course instanceof Types.ObjectId
          ? enrollment.course
          : new Types.ObjectId((enrollment.course as any).toString()),
      );
      ...
      const coursesWithEnrollmentDates = courses.map((course) => {
        ...
        if (enrollment) {
          courseObj.createdAt =
            enrollment['createdAt'] || enrollment['_doc']?.createdAt;
        }
        return courseObj;
      });
      ...
      this.logger.error(
        `Error getting enrolled courses: ${error.message}`,
        error.stack,
      );
      throw error;
    }
    ...
    try {
      ...
      const isCourseTeacher =
        ...
        ((course.teacher &&
          ...
          (course.teacher as any)._id.toString() === userId) || // Comparar _id si teacher está populado
          (course.teacher &&
            !(course.teacher as any)._id &&
            course.teacher.toString() === userId) || // Comparar directamente si teacher es solo ObjectId
          (course.teachers &&
      ...
      this.logger.warn(
        `[getCourseForUser] ACCESO DENEGADO FINAL. Curso ID: ${id}, Usuario ID: ${userId}, Rol: ${normalizedRole}`,
      );
      throw new UnauthorizedException(
        'No tienes permiso para ver este curso o requiere autenticación.',
      );
      ...
      this.logger.error(
        `[getCourseForUser] ERROR EXCEPCION. Curso ID: ${id}, Error: ${error.message}`,
        error.stack,
      );
  ...
  async addPaymentToEnrollment(
    ...
    if (wasInactive) {
      ...
      try {
        ...
        updatePromises.push(student.save().then(() => {}));

        // Esperar a que todas las actualizaciones se completen
        await Promise.all(updatePromises);
      } catch (error) {
        this.logger.error(
          `Error actualizando referencias al reactivar enrollment: ${error.message}`,
          error.stack,
        );
  ...
  async createEnrollmentForPayment(
    courseId: string,
    studentId: string,
    userId: string,
  ): Promise<any> {
    try {
      ...
      const isStudentInCourse =
        course.students &&
        course.students.some((s) => s.toString() === studentId);
      const isStudentEnrolled =
        student.enrolledCourses &&
        student.enrolledCourses.some((c) => c.toString() === courseId);

      if (!isStudentInCourse && !isStudentEnrolled) {
        this.logger.error(
          `Student ${studentId} is not associated with course ${courseId}`,
        );
        return null;
      }
      ...
      this.logger.error(
        `Error creating enrollment for payment: ${error.message}`,
        error.stack,
      );
      return null;
    }
   ...
   */
  async migrateExistingCourses(): Promise<{
    success: boolean;
    message: string;
    updatedCount: number;
  }> {
    this.logger.log(
      'Iniciando migración para agregar campos de promoción a cursos existentes',
    );
    ...
    try {
      ...
      this.logger.log(
        `Encontrados ${coursesWithoutPromoFields} cursos sin campos de promoción`,
      );
      ...
      this.logger.log(
        `Migración completada. Actualizados ${updateResult.modifiedCount} cursos`,
      );
  ...
  async getPaymentHistoryForMonth(
    courseId: string,
    studentId: string,
    month: string,
    userId: string,
    ...
    try {
      ...
      const monthPayments = enrollment.paymentHistory.filter(
      ...
      const result = {
        payments: sortedPayments.map((payment, index) => {
          ...
          const consistentId = crypto
            .createHash('md5')
            .update(paymentDataString)
            .digest('hex')
            .substring(0, 24);
  ...
  async updatePayment(
    ...
    try {
      ...
      if (!payment) {
        this.logger.error(
          `Payment with ID ${paymentId} not found in enrollment ${enrollment._id}`,
        );
        throw new NotFoundException(
          'Payment not found. Please refresh the payment list and try again.',
        );
      }
