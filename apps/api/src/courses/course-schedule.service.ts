...
export class CourseScheduleService {
  ...
  constructor(
    @InjectModel(CourseSchedule.name)
    private courseScheduleModel: Model<CourseScheduleDocument>,
  ...
  async createSchedule(
    ...
    if (
      createScheduleDto.scheduleTimes &&
      createScheduleDto.scheduleTimes.length > 1
    ) {
  ...
  async updateSchedule(
    ...
    if (
      updateScheduleDto.scheduleTimes &&
      updateScheduleDto.scheduleTimes.length > 1
    ) {
  ...
  async deleteSchedule(courseId: string): Promise<boolean> {
    const result = await this.courseScheduleModel.deleteOne({
      course: courseId,
    });
    return result.deletedCount > 0;
  }

  async getUpcomingClasses(timeWindow: {
    start: Date;
    end: Date;
  }): Promise<any[]> {
    // Get all active schedules with notifications enabled
    const schedules = await this.courseScheduleModel
      .find({
    ...
    for (const schedule of schedules) {
      if (!schedule.course || !(schedule.course as any).school) {
        this.logger.warn(
          `Schedule ${schedule._id} missing course or school data`,
        );
        continue;
      }
      ...
      for (const scheduleTime of schedule.scheduleTimes) {
        ...
        const nextClassTime = this.calculateNextClassTime(
          scheduleTime,
          schoolTimezone,
        );

        if (
          nextClassTime >= timeWindow.start &&
          nextClassTime <= timeWindow.end
        ) {
  ...
  private calculateNextClassTime(
    scheduleTime: any,
    ...
    classDateInSchoolTz.setUTCDate(
      classDateInSchoolTz.getUTCDate() + daysUntilClass,
    );
    classDateInSchoolTz.setUTCHours(hours, minutes, 0, 0);

    // Convert back to UTC
    const classDateUTC = new Date(
      classDateInSchoolTz.getTime() + timezoneOffset * 60000,
    );
  ...
  async cleanupOrphanedSchedules(): Promise<void> {
    try {
      ...
      if (orphanedSchedules.length > 0) {
        await this.courseScheduleModel.deleteMany({
          _id: { $in: orphanedSchedules },
        });
        this.logger.log(
          `Cleaned up ${orphanedSchedules.length} orphaned course schedules`,
        );
      }
    } catch (error) {
