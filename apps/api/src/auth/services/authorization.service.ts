...
export class AuthorizationService {
  ...
  async getUserRoleInSchool(
    ...
    if (
      String(user.role).toLowerCase() === UserRole.SUPER_ADMIN.toLowerCase()
    ) {
  ...
  async isSchoolAdmin(userId: string, schoolId: string): Promise<boolean> {
    ...
    if (
      ...
      user.schoolRoles?.some(
        (sr) =>
          sr.schoolId.toString() === schoolId &&
          sr.role === UserRole.ADMINISTRATIVE,
      )
    ) {
      return true;
    }
  ...
  async canManageUserInSchool(
    ...
    this.logger.log(
      `Checking if user ${managerId} (role: ${manager.role}) can manage user ${targetUserId} in school ${schoolId}`,
    );

    // Super admin puede gestionar a cualquier usuario en cualquier escuela
    if (
      String(manager.role).toLowerCase() === UserRole.SUPER_ADMIN.toLowerCase()
    ) {
    ...
    if (
      String(managerRole).toLowerCase() === UserRole.SCHOOL_OWNER.toLowerCase()
    ) {
      return (
        String(targetUser.role).toLowerCase() !==
    ...
    if (String(managerRole).toLowerCase() === UserRole.ADMIN.toLowerCase()) {
      ...
      return (
        String(targetRole).toLowerCase() === UserRole.TEACHER.toLowerCase() ||
    ...
    if (String(managerRole).toLowerCase() === UserRole.TEACHER.toLowerCase()) {
      ...
      if (String(targetRole).toLowerCase() === UserRole.STUDENT.toLowerCase()) {
        ...
        const professorCourseIds = professorCourses.map((c) =>
          c._id.toString(),
        );
  ...
  async assignRoleInSchool(
    ...
    try {
      ...
      this.logger.log(
        `Asignando rol ${normalizedRole} al usuario ${userId} en la escuela ${schoolId}`,
      );
      ...
      if (
        ...
        this.logger.error(
          `ID inválido - userId: ${userId}, schoolId: ${schoolId}`,
        );
        return false;
      }
      ...
      this.logger.log(
        `Estado actual de schoolRoles: ${JSON.stringify(user.schoolRoles)}`,
      );

      if (existingRoleIndex >= 0) {
        // Si ya tiene un rol, actualizarlo
        this.logger.log(
          `Actualizando rol existente en índice ${existingRoleIndex}`,
        );
      ...
      // Guardar los cambios
      this.logger.log(
        `Guardando usuario con schoolRoles actualizados: ${JSON.stringify(user.schoolRoles)}`,
      );
      await user.save();
      ...
      if (normalizedRole === 'school_owner') {
        ...
        await this.schoolModel.updateOne({ _id: schoolId }, { admin: userId });
      ...
      // Actualizar las colecciones regulares
        ...
        const hasTeacher = school.teachers.some(
        ...
        const hasStudent = school.students.some(
          (id) => id.toString() === userId,
        );
        if (!hasStudent) {
          await this.schoolModel.updateOne(
        ...
        const hasAdministrative = school.administratives.some(
          (id) => id.toString() === userId,
        );
