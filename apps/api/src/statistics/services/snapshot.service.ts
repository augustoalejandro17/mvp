...
export class SnapshotService {
  ...
  constructor(
    ...
    private dailySnapshotModel: Model<DailySnapshotDocument>,
    ...
    try {
      ...
      for (const school of schools) {
        try {
          ...
          processedCount++;
          this.logger.log(
            `Snapshot generated for school: ${school.name} (${school._id})`,
          );
        } catch (error) {
          errorCount++;
          this.logger.error(
            `Failed to generate snapshot for school ${school._id}: ${error.message}`,
            error.stack,
          );
        }
      }
      ...
      this.logger.log(
        `Daily snapshot generation completed. Processed: ${processedCount}, Errors: ${errorCount}`,
      );
      await this.cleanupOldSnapshots();
    } catch (error) {
      this.logger.error(
        'Fatal error in daily snapshot generation:',
        error.message,
        error.stack,
      );
    }
  ...
  async generateSnapshotForSchool(
    ...
    const startOfDay = new Date(
      startOfDayLocal.getTime() + timezoneOffset * 60000,
    );
    ...
    const existingSnapshot = await this.dailySnapshotModel
      .findOne({
        academyId,
    ...
    if (existingSnapshot) {
      this.logger.warn(
        `Snapshot already exists for school ${academyId} on ${localDateStr}`,
      );
      return existingSnapshot;
    }

    // Get all data in parallel
    const [revenueData, attendanceData, enrollmentData, courseData] =
      await Promise.all([
    ...
    const estimatedMaxSeats = Math.max(
      totalEnrollments * 1.2,
    ...
    const avgRetentionDays =
      activeEnrollments.length > 0
        ? Math.round(
            activeEnrollments.reduce((sum, e) => {
              const daysSinceEnrollment = Math.floor(
                (startOfDay.getTime() -
                  new Date(e.createdAt || e.updatedAt).getTime()) /
                  (1000 * 60 * 60 * 24),
              );
    ...
    const droppedToday = enrollmentData.filter(
      (e) =>
        !e.isActive &&
    ...
    const snapshot = new this.dailySnapshotModel({
      ...
      droppedToday,
      avgRetentionDays,
      byProfessor: await this.getAggregatedByProfessor(
        academyId,
        startOfDay,
        endOfDay,
      ),
      byCourse: await this.getAggregatedByCourse(
        academyId,
        startOfDay,
        endOfDay,
      ),
      byCategory: await this.getAggregatedByCategory(
        academyId,
        startOfDay,
        endOfDay,
      ),
  ...
  private async getCourseData(academyId: Types.ObjectId) {
    return await this.courseModel
      ...
      .select('students teacher categories')
      .exec();
  }

  private async getAggregatedByProfessor(
    academyId: Types.ObjectId,
    startOfDay: Date,
    endOfDay: Date,
  ): Promise<AggregatedRow[]> {
    const professorData = await this.courseModel
      .aggregate([
        {
        ...
        {
          $unwind: {
            path: '$enrollments.paymentHistory',
  ...
  private async getAggregatedByCourse(
    academyId: Types.ObjectId,
    startOfDay: Date,
    endOfDay: Date,
  ): Promise<AggregatedRow[]> {
  ...
  private async getAggregatedByCategory(
    academyId: Types.ObjectId,
    startOfDay: Date,
    endOfDay: Date,
  ): Promise<AggregatedRow[]> {
  ...
  private async cleanupOldSnapshots(): Promise<void> {
    ...
    try {
      const result = await this.dailySnapshotModel
        .deleteMany({
          date: { $lt: cutoffDate },
        })
        .exec();
      ...
      this.logger.log(
        `Cleaned up ${result.deletedCount} old snapshots older than ${cutoffDate.toISOString()}`,
      );
    } catch (error) {
      this.logger.error(
  ...
  async generateSnapshotManually(
    schoolId: string,
    date?: Date,
  ): Promise<DailySnapshot> {
    const targetDate = date || this.getYesterday();
    this.logger.log(
      `Manually generating snapshot for school ${schoolId} on ${targetDate.toISOString()}`,
    );
