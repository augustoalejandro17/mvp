...
export class GamificationIntegrationService {
    ...
    try {
      ...
      if (userPoints && videoData.watchedPercentage >= 90) {
        ...
        if (hasCompletionPoints) {
          this.logger.log(
            `Video completion points already awarded for class ${classId}, skipping duplicate award`,
          );
          return;
        }
      }
  ...
  async handleClassAttendance(
    ...
    try {
      ...
      // Participation bonus
      if (
        attendanceData.participationScore &&
        attendanceData.participationScore > 7
      ) {
  ...
  async handleAssignmentCompletion(
    ...
    try {
      ...
      this.logger.error(
        `Error handling assignment completion: ${error.message}`,
      );
    }
  }
  ...
  async handleCourseCompletion(
    ...
    try {
      const completionPercentage =
        (courseData.attendedClasses / courseData.totalClasses) * 100;
  ...
  private async updateAttendanceStreak(
    userId: string,
    schoolId: string,
  ): Promise<void> {
    try {
  ...
  async initializeNewUserGamification(
    ...
    try {
      ...
      this.logger.error(
        `Error initializing new user gamification: ${error.message}`,
      );
    }
  }

  async handleDailyLogin(userId: string, schoolId: string): Promise<void> {
    try {
