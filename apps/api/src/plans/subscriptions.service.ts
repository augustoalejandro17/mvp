...
export class SubscriptionsService {
  ...
  async create(
    ...
    const plan = await this.planModel
      .findById(createSubscriptionDto.plan)
      .exec();
    if (!plan) {
    ...
    if (existingSubscription) {
      throw new BadRequestException(
        'La escuela ya tiene una suscripción activa',
      );
    }
    ...
    await this.schoolModel
      .findByIdAndUpdate(createSubscriptionDto.school, {
  ...
  async remove(id: string): Promise<any> {
    ...
    await this.schoolModel
      .findByIdAndUpdate(subscription.school, {
  ...
  async canAddUserToSchool(
    schoolId: string,
  ): Promise<{ canAdd: boolean; available: number; message: string }> {
    try {
      // Buscar la escuela
  ...
  async canEnrollUserToCourse(
    ...
    try {
      ...
      const school = await this.schoolModel
        .findOne({ courses: { $in: [courseId] } })
        .exec();
      if (!school) {
        return {
          canEnroll: false,
          message: 'Escuela no encontrada para este curso',
        };
      }

      // Buscar la suscripción activa de la escuela
      const subscription = await this.findSubscriptionForSchool(
        school._id.toString(),
      );
      if (!subscription) {
        return {
      ...
      const userCourses = await this.courseModel
        .find({
  ...
  private async findSubscriptionForSchool(
    schoolId: string,
    ...
    try {
      ...
      if (!subscription) {
        throw new NotFoundException(
          'La escuela no tiene una suscripción activa',
        );
      }
      ...
      await this.schoolModel
        .findByIdAndUpdate(schoolId, { storageUsedGb: newStorageUsed })
        .exec();
  ...
  async findSchoolBySubscription(subscriptionId: string): Promise<any> {
    const school = await this.schoolModel
      .findOne({ activeSubscription: subscriptionId })
      .exec();
    return school;
  }
}
