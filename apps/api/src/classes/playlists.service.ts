...
export class PlaylistsService {
  ...
  async findByCourse(
    ...
    try {
      ...
      if (!userId || !normalizedRole || normalizedRole === 'student') {
      ...
      for (const playlist of playlists) {
        ...
        if (playlistObj.classes && playlistObj.classes.length > 0) {
          ...
          playlistObj.classes =
            await this.generateSignedUrlsForClasses(filteredClasses);
        }
  ...
  async reorderPlaylistsInCourse(
    courseId: string,
    playlistIds: string[],
    userId: string,
    userRole: string,
  ...
  private async filterClassesForUser(
    ...
    const course = await this.courseModel.findById(courseId);
    if (course && userRole?.toLowerCase() === 'teacher') {
      const isCourseTeacher =
